 ...
 */
export async function uploadImage(imageUrl: string, folder: string = 'astra-images'): Promise<string> {
  // Check if we're in a browser environment
  if (typeof window !== 'undefined') {
    throw new Error('This function can only be used on the server side. Please use an API route to upload images.');
  }

  // Check if Cloudinary is configured
  if (!process.env.CLOUDINARY_API_KEY || !process.env.CLOUDINARY_API_SECRET || !process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME) {
    throw new Error('Cloudinary is not configured. Please add the required environment variables.');
  }
...
export function getOptimizedImageUrl(imageUrl: string, width?: number, height?: number): string {
  try {
    ...
    if (!process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME) {
      console.warn('Cloudinary cloud name is not configured. Returning original URL.');
      return imageUrl;
    }
...
export const cloudinary = typeof window === 'undefined' ? cloudinaryServer : undefined; 