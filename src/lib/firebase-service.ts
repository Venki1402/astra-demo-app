...
export const signInWithGoogle = async () => {
  try {
    ...
    if (error.code === 'auth/configuration-not-found') {
      console.error('Firebase Auth Configuration Error: Make sure Google Auth is enabled in your Firebase project');
      console.error('Please visit the Firebase console and enable Google authentication');
      console.error('Also verify that your domain is in the authorized domains list');
    } else if (error.code === 'auth/popup-closed-by-user') {
      console.error('Sign-in popup was closed before completing the sign-in process');
    } else if (error.code === 'auth/popup-blocked') {
      console.error('Sign-in popup was blocked by your browser. Please allow popups for this site');
    }
  ...
  try {
    ...
    if (parentId) {
      ...
      if (parentDoc.exists()) {
        ...
        versionHistory = [...(parentData.versionHistory || [parentId]), parentId];
...
export const searchImagesByPrompt = async (searchTerm: string): Promise<ImageData[]> => {
  try {
...
export const searchImagesBySimilarity = async (imageFile: File): Promise<ImageData[]> => {
  try {
    // Get all images
    const allImages = await getAllImages();
...
export const getDesignerDetails = async (userId: string): Promise<UserData | null> => {
  try {
...
export const voteForImage = async (imageId: string, userId: string): Promise<boolean> => {
  try {
...
export const hasUserVotedForImage = async (imageId: string, userId: string): Promise<boolean> => {
  try {
    const imageDoc = await getDoc(doc(db, 'images', imageId));
...
export const getTopVotedImages = async (limit: number = 10): Promise<ImageData[]> => {
  try {
...
export const getImageVersions = async (imageId: string): Promise<ImageData[]> => {
  try {
    // Get the image
    ...
    if (imageData.parentId) {
      ...
      const versions = versionDocs.docs.map(doc => ({ ...doc.data(), id: doc.id }) as ImageData);
...
export const getImageAncestors = async (imageId: string): Promise<ImageData[]> => {
  try {
