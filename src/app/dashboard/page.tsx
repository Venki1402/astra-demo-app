...
import { getUserImages, ImageData, searchImagesByPrompt, searchImagesBySimilarity } from '@/lib/firebase-service';
...
export default function Dashboard() {
  ...
  const [selectedImageForReprompt, setSelectedImageForReprompt] = useState<ImageData | null>(null);
  ...
  const handleImageSearch = async (file: File) => {
    ...
    try {
      ...
      const userSimilarImages = allSimilarImages.filter(image => image.userId === user.uid);
      setFilteredImages(userSimilarImages);
      
      // If no results found, show a message
      if (userSimilarImages.length === 0) {
        setError('No similar images found in your collection. Try a different image or search term.');
      }
    } catch (error) {
      ...
      <main className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 container mx-auto px-4 py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(6)].map((_, index) => (
              <div key={index} className="border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-4">
        ...
        {isLoading ? (
          ...
          <ImageGallery 
            ...
            title={searchMode === 'text' 
              ? (searchTerm ? `Search Results for "${searchTerm}"` : "Your Images")
