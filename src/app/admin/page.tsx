...
import { getAllImages, ImageData, searchImagesByPrompt, searchImagesBySimilarity, getDesignerDetails, UserData } from '@/lib/firebase-service';
...
export default function AdminDashboard() {
  ...
  const [designers, setDesigners] = useState<{ id: string; name: string; email: string; imageCount: number }[]>([]);
  const [selectedDesigner, setSelectedDesigner] = useState<UserData | null>(null);
  const [isDesignerModalOpen, setIsDesignerModalOpen] = useState(false);
  const [isVersionHistoryOpen, setIsVersionHistoryOpen] = useState(false);
  const [selectedVersionImageId, setSelectedVersionImageId] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      try {
        ...
        const designerMap = new Map<string, { name: string; email: string; imageCount: number }>();
        ...
        const designerList = Array.from(designerMap.entries()).map(([id, data]) => ({
          id,
          ...data
        }));
  ...
  const handleSearch = async (term: string) => {
    ...
    if (!term.trim()) {
      ...
      if (designerFilter) {
        const filtered = images.filter(image => image.userId === designerFilter);
    ...
    try {
      ...
      if (designerFilter) {
        const filtered = searchResults.filter(image => image.userId === designerFilter);
        setFilteredImages(filtered);
      } else {
  ...
  const handleImageSearch = async (file: File) => {
    ...
    try {
      ...
      if (searchResults.length === 0) {
        setError('No similar images found. Try a different image or search term.');
      }
    } catch (error) {
  ...
  const handleImageDeleted = async (imageId: string) => {
    ...
    try {
      ...
      setError('Image was deleted, but there was an error refreshing the list.');
    }
  };
    ...
    return (
      <main className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 container mx-auto px-4 py-12">
          <h1 className="text-4xl font-bold mb-8">Loading Admin Dashboard...</h1>
    ...
    <main className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 container mx-auto px-4 py-12">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-4">
            ...
            <Button onClick={() => router.push('/admin-setup')} variant="neutral">
              Admin Setup
            </Button>
          </div>
        </div>
        
        <Tabs defaultValue="images" className="mb-8" onValueChange={handleTabChange}>
          ...
          <TabsContent value="images">
                  ...
                  {designerFilter && (
                    <div className="mt-4 flex items-center gap-2">
                      ...
                      <Badge className="px-2 py-1">
                        {designers.find(d => d.id === designerFilter)?.name || 'Unknown'}
                      </Badge>
                      <Button 
                        variant="neutral" 
                        size="sm"
            ...
            {isLoading ? (
              ...
              <ImageGallery 
                ...
                title={searchMode === 'text' 
                  ? (searchTerm ? `Search Results for "${searchTerm}"` : "All Images")
              ...
              {designers.map(designer => (
                ...
                >
                  <CardContent className="p-6">
                    ...
                    <p className="text-gray-500 mb-4">{designer.email}</p>
                    <div className="flex justify-between items-center">
                      <Badge className="px-2 py-1">
                        {designer.imageCount} {designer.imageCount === 1 ? 'image' : 'images'}
                      </Badge>
      ...
      <Dialog open={isVersionHistoryOpen} onOpenChange={setIsVersionHistoryOpen}>
        <DialogContent className="max-w-3xl bg-white p-6 border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold">Image Version History</DialogTitle>
          </DialogHeader>
      ...
      <footer className="bg-black text-white py-6">
        <div className="container mx-auto px-4 text-center">
          <p>&copy; {new Date().getFullYear()} Astra Labs. All rights reserved.</p>
        </div>
      </footer>
    </main>
  );
