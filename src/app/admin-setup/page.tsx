...
export default function AdminSetup() {
  ...
  const handleSetAdmin = async () => {
    ...
    try {
      ...
      if (success) {
        setStatus('success');
        setMessage(`User ${userId} has been set as admin successfully. Note: The user will need to log out and log back in for the changes to take effect.`);
      } else {
        setStatus('error');
  ...
  const handleSetCurrentUserAsAdmin = async () => {
    ...
    try {
      ...
      if (success) {
        setStatus('success');
        setMessage(`You (${user.displayName || user.email}) have been set as admin successfully. Please log out and log back in for the changes to take effect.`);
      } else {
        setStatus('error');
            ...
            <CardContent className="p-6">
              ...
              <div className="space-y-4">
                ...
                <Button
                  ...
                  className="w-full"
                >
                  {status === 'loading' ? 'Setting...' : 'Set Yourself as Admin'}
                </Button>
