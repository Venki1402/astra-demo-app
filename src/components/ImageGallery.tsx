...
export default function ImageGallery({ 
  ...
  const handleDownload = async (imageUrl: string, prompt: string, event: React.MouseEvent) => {
    event.stopPropagation();
    ...
    <div className="w-full">
        ...
        {localImages.map((image) => (
          ...
          >
            <CardContent className="p-4">
              <div className="aspect-square overflow-hidden border-2 border-black mb-2 relative">
                <img 
                  src={image.imageUrl} 
                  alt={image.prompt} 
                ...
                {showVersions && image.version && (
                  <div className="absolute top-2 right-2">
                    <Badge className="bg-white">v{image.version}</Badge>
                  </div>
                )}
              </div>
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">By {image.userName}</p>
                </div>
                <div className="flex gap-2">
                  {showVoting && image.id && (
                    ...
                    >
                      <Heart className={`h-4 w-4 ${votedImages[image.id] ? 'fill-pink-500 text-pink-500' : ''}`} />
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                {showVersions && selectedImage.version && (
                  <Badge className="mt-2">Version {selectedImage.version}</Badge>
                )}
              </div>
              <div className="flex items-center gap-2">
                {showVoting && selectedImage.id && (
                  ...
                  >
                    <Heart className={`h-4 w-4 ${votedImages[selectedImage.id] ? 'fill-pink-500 text-pink-500' : ''}`} />
      ...
      <AlertDialog open={!!imageToDelete} onOpenChange={(open) => !open && setImageToDelete(null)}>
        <AlertDialogContent className="border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-white">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-2xl font-bold">Are you sure you want to delete this image?</AlertDialogTitle>
            <AlertDialogDescription className="text-base">
              This action cannot be undone. This will permanently delete the image from the database.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex gap-4">
            <AlertDialogCancel onClick={cancelDelete} className="border-2 border-black">Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={confirmDelete} className="bg-red-500 hover:bg-red-600 border-2 border-black text-white">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
